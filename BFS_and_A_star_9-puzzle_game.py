...
def A_star_Taquin(start, goal, graph):
	...
	fCost[start] = heuristic_Taquin(start, goal)
	bestPrevious = {}
	count = 0
	end = False
	while ongoing and not end:
		...
		for n in unvisited_transmissions(bestNode, processed, graph)[0]:
			...
			elif new_cost_to_n >= lowestCostTo[n]: continue # use 'elif' to assure that the n exists
			lowestCostTo[n] = new_cost_to_n
			bestPrevious[n] = bestNode
			fCost[n] = lowestCostTo[n] + heuristic_Taquin(n, goal)
			if count%5000==0: print('States visited: %d\t...' % count)
			count += 1


def unvisited_transmissions(e, visited, graph=None, get_graph=False): 
	''' return the next possible and unvisited states '''
...
def heuristic_Taquin(current, e_sol): # get heuristic by the sum of Manhattan distance
	sum_distance = 0
	for v in e_sol:
...
def main():
	...
	parser.add_argument('-g', action='store_true', help='generate and store the state graph of Taquin, not necessary if the graph is already generated')
	...
	Taquin_init = [i for i in range(1,10)] # 9 is the blank square
